        ...
        public IOdooRecord Create(object values)
        {
            ...
            else
            {
                ...
                dict = values
                    .GetType()
                    .GetProperties()
                    .ToDictionary(p => ToSnakeCase(p.Name), p => p.GetValue(values));
            }
        ...
        private IOdooRecord CreateFromDictionary(Dictionary<string, object?> values)
        {
            ...
            foreach (var (fieldName, value) in values)
            {
                ...
                if (_schema.Fields.TryGetValue(fieldName, out var fieldSchema))
                {
                    ...
                    genericMethod.Invoke(
                        _env.Columns,
                        new object[] { _schema.Token, newId, fieldSchema.Token, value }
                    );
                }
                else
                {
                    throw new KeyNotFoundException(
                        $"Field '{fieldName}' not found on model '{_modelName}'"
                    );
                }
            }
            ...
            if (_env is OdooEnvironment odooEnv)
            {
                ...
                if (mainType != null)
                {
                    ...
                    var createMethod = typeof(IEnvironment)
                        .GetMethod("CreateRecordSet")!
                        .MakeGenericMethod(mainType);
            ...
            throw new InvalidOperationException(
                $"Could not create record for model '{_modelName}'"
            );
        }
